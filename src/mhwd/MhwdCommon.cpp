 ...
 *
 *  Change 2019 Angelo Scarnà <angelo.scarna@codelinsoft.it>
...
{
    return QString( tr ( "Condres Hardware Detection graphical user interface" ) );
}
...
bool MhwdCommon::install(const QVariantMap& args)
{
    QProcess mhwd;
    ...
    if ( !mhwd.waitForStarted() )
        return QMessageBox::warning(nullptr,"Condres Control Center", "Can't start process install");

    if ( !mhwd.waitForFinished( 600000 ) )
        return QMessageBox::warning(nullptr,"Condres Control Center", "Can't finish process install");

    return QMessageBox::warning(nullptr,"Condres Control Center", "Install successfull...");;
}

bool MhwdCommon::remove(const QVariantMap& args)
{
    QProcess mhwd;
    mhwd.start( "/usr/bin/mhwd", args["arguments"].toStringList() );
    connect( &mhwd, &QProcess::readyRead,
    ...
    return QMessageBox::warning(nullptr,"Condres Control Center", "Remove successfull...");;
}
